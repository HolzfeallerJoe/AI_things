 ...
 */
export function resolveFigmaCredentials(options = {}) {
    ...
    const accessToken = profile === 'team_vault'
        ? env.FIGMA_TEAM_VAULT_ACCESS_TOKEN ?? env.FIGMA_ACCESS_TOKEN
        : env.FIGMA_ACCESS_TOKEN;
    if (!accessToken) {
        if (profile === 'team_vault') {
            throw new Error('Missing Figma access token. Set FIGMA_TEAM_VAULT_ACCESS_TOKEN (or fallback FIGMA_ACCESS_TOKEN).');
        }
        throw new Error('Missing Figma access token. Set FIGMA_ACCESS_TOKEN.');
    }
    const teamId = profile === 'team_vault'
        ? env.FIGMA_TEAM_VAULT_TEAM_ID ?? env.FIGMA_TEAM_ID
        : env.FIGMA_TEAM_ID;
    const userId = profile === 'team_vault'
