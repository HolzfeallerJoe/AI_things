...
void main() {
  ...
  group('Zeitraum', () {
    ...
    test('laesst leere Tage weg', () {
      final snapshot = snapshotOf(stateWith(appointments: [
        Appointment(
          id: 'a1',
          title: 'Zahnarzt',
    ...
    test('ist als JSON verschickbar', () {
      final snapshot = snapshotOf(stateWith(
        ...
        appointments: [
          Appointment(id: 'a1', title: 'Zahnarzt', when: heute.add(const Duration(hours: 9)))
        ],
      ));
  ...
  group('Ein Tag', () {
    test('traegt Aufgaben mit Farbe und Haken', () {
      final snapshot = snapshotOf(stateWith(tasks: [
        Task(
          id: 't1',
          title: 'Blumen gießen',
          startDate: heute,
    ...
    test('zaehlt offene Aufgaben wie das Dashboard, ohne Stufe 3', () {
      final state = stateWith(tasks: [
        Task(id: 't1', title: 'Wichtig', startDate: heute, priority: Priority.hoch),
        Task(id: 't2', title: 'Leise', startDate: heute, priority: Priority.niedrig),
      ]);
    ...
    test('merkt sich Notizen', () {
      final snapshot = snapshotOf(stateWith(notes: [
        Note(id: 'n1', title: 'Gedanke', body: '', updatedAt: now, date: heute),
      ]));
    ...
    test('merkt sich Feiertage – die rechnet das Widget nicht selbst', () {
      ...
      final snapshot =
          buildWidgetSnapshot(stateWith(), now: DateTime(2026, 9, 20, 10, 0));
      final day =
  ...
  group('Vorausgerechnete Tage', () {
    test('kennen die Wiederholung, damit das Widget sie nicht kennen muss', () {
      ...
      for (final offset in [0, 3, 6, 9]) {
        expect(dayOf(snapshot, heute.add(Duration(days: offset))), isNotNull,
    ...
    test('sagen dazu, was nur mitgeschleppt ist', () {
      ...
      expect(
        (inEinerWoche['tasks'] as List)
            .firstWhere((t) => t['title'] == 'Wöchentlich')['over'],
        isFalse,
      );
    });

    test('markieren im Raster trotzdem nur den Tag, an dem etwas faellig ist',
        () {
      ...
      final heutig = snapshotOf(stateWith(tasks: [
        Task(id: 't2', title: 'Heute faellig', startDate: heute),
      ]));
      expect((dayOf(heutig, heute)!['mark'] as Map)['done'], isFalse);
      expect(dayOf(heutig, heute.add(const Duration(days: 3)))!['mark'], isNull);
    });
    ...
    test('lassen eine erledigte einmalige Aufgabe am Tag stehen, nicht spaeter',
        () {
      final snapshot = snapshotOf(stateWith(tasks: [
        Task(
          id: 't1',
      ...
      expect((dayOf(snapshot, heute)!['tasks'] as List).single['done'], isTrue);
    ...
    test('rechnen ueber die Zeitumstellung hinweg Tag fuer Tag', () {
      ...
      for (final tag in [
        ...
        expect(days.containsKey(dateKey(tag)), isTrue,
